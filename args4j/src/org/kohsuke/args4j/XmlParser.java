 ...
 * </pre>
 * 
 * Exactly one of the attributes 'field' or 'method' must be set. The 'handler' value specifies a
	...
	public void parse(InputSource xml, CmdLineParser parser, Object bean) {
		try {
			...
			for (ConfigElement ce : config.options) {
				...
				parser.addOption(
						Setters.create(parser, findMethodOrField(bean, ce.field, ce.method), bean),
						option);
			}
